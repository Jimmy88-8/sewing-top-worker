...
| Weather | Apple Weather 风格仪表盘：小时/10 日预报、降水图、UV、日落、风力等；地点标签默认泛化 |
| Calculator | macOS 风格计算器，支持键盘 |
| Calendar | Apple Calendar 风格日/周/月/年视图，本地事件编辑与隐私友好的示例日程 |
...
| 路径 | 说明 |
|---|---|
| `GET /api/quotes?extra=PLTR,BABA` | 全部标的报价（含 sparkline、52 周高低、来源标注）；`extra` 可附加任意 Yahoo 代码 |
| `GET /api/history?symbol=AAPL&range=1D` | OHLCV K 线（1D/5D/1M/6M/1Y），支持任意 Yahoo 代码 |
| `GET /api/search?q=apple` | 标的搜索（Yahoo 查询，离线回退本地注册表） |
