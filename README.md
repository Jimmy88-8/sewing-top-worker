# sewing.top — SewingOS + Market Terminal

macOS 风格的网页桌面系统（SewingOS），内置 Bloomberg Terminal 风格的金融终端（Market Terminal），**接入真实行情数据**。运行在 Cloudflare Workers + Static Assets 上。
...
| 应用 | 说明 |
|---|---|
| Market Terminal | Bloomberg 风终端（见下） |
